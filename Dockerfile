...
FROM registry.api.weibo.com/weibo_rd_if/breeze_builder:1.54.0.1 AS builder
WORKDIR /breeze
COPY . .
ADD --chown=rust:rust . ./
RUN sed -i -e "s/BREEZE_VERSION_PLACE_HOLDER/`git rev-parse --abbrev-ref HEAD`@`git rev-parse --short HEAD`@`date +"%Y-%m-%d"`/g" context/src/lib.rs
...
CMD ["--discovery", "vintage://static.config.api.weibo.com", "--snapshot", "/data1/breeze/snapshot", "--service-path", "/data1/breeze/socks", "--log-dir", "/data1/breeze/logs", "--metrics-url", "logtailer.monitor.weibo.com:8333", "--upgrade", "--port", "9984", "--metrics-probe", "10.10.10.10:53"]
